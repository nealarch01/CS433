...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Author: Neal Archival and Ahmed Ali" << std::endl;
    ...
    if (!in2.is_open()) {
        std::cout << "Cannot open large_ref.txt" << std::endl;
        return 1;
    }
    std::vector<int> big_refs;
    while (in2 >> val) {
        big_refs.push_back(val);
    }
    in2.close();
    // FIFOReplacement vm2(num_pages, num_frames);
    FIFOReplacement *vm2 = new FIFOReplacement(num_pages, num_frames);
    for (std::vector<int>::const_iterator it = big_refs.begin(); it != big_refs.end(); ++it) {
        int page_num = (*it) >> page_offset_bits;
