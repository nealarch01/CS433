...
bool Replacement::access_page(int page_num, bool is_write)
{
    ...
    // Default return (no page fault occured)
    touch_page(page_num);
    return false;
}
...
void Replacement::print_statistics() const {
		...
		std::cout << "Number of references: \t\t" << num_of_refs << std::endl;
		std::cout << "Number of page faults: \t\t" << num_of_page_faults << std::endl;
		std::cout << "Number of page replacements: \t"  << num_of_page_replacements << std::endl;
}